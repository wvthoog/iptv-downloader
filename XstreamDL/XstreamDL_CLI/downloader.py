...
from XstreamDL.XstreamDL_CLI.models.segment import Segment
from XstreamDL.XstreamDL_CLI.util.decryptors.aes import CommonAES
from XstreamDL.XstreamDL_CLI.util.texts import t_msg
from XstreamDL.XstreamDL_CLI.log import setup_logger
from re import sub
from XstreamDL.XstreamDL_CLI.util.concat import Concat
    ...
    def try_concat_streams(self, streams: List[Stream], selected: List[str]):
        for stream in streams:
            ...
            if self.args.live is True and self.args.disable_auto_concat is False:
                print('Downloader args', self.args)
                stream.concat(self.args)
        if self.args.merge is True:
            print('Filenames:', self.args.merge_files)
            audio_encr = self.args.merge_files[0]
            video_encr = self.args.merge_files[1]
            merged = (audio_encr.parent / f"{audio_encr.stem.replace('audio_', '')}{sub(r'^.*?video_', '', video_encr.stem)}_merged.mp4").absolute()

            audio_decr = (audio_encr.parent / f"{audio_encr.stem}_decrypted{audio_encr.suffix}").absolute()
            video_decr = (video_encr.parent / f"{video_encr.stem}_decrypted{video_encr.suffix}").absolute()

            Concat.call_ffmpeg_merge(audio_decr, video_decr, merged, self.args)
