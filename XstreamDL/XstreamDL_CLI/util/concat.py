...
class Concat:
    ...
    @staticmethod
    def call_ffmpeg_merge(audio: Path, video: Path, merged, args: CmdArgs):
        assert audio.exists() and video.exists() is True, f'Files dont exist ! -> {audio} and {video}'
        #print('merged:', merged)

        if args.overwrite is False and merged.exists():
            print(t_msg.merged_file_exists_skip)
            return
        if platform.system() == 'Windows':
            _cmd = f'""{args.ffmpeg}" -hide_banner -i {audio} -i {video} -c copy "{merged.as_posix()}""'
        else:
            _cmd = f'"{args.ffmpeg}" -hide_banner -i {audio} -i {video} -c copy "{merged.as_posix()}"'
        os.system(_cmd)
        if args.enable_auto_delete:
            if merged.exists() and merged.stat().st_size > 0:
                os.remove(audio, video)
